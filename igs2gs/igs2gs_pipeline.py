...
class InstructGS2GSPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        # to compute igs2gs_steps
        self.gs_idx = 0
        self.gs_steps = self.config.gs_steps
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if (not self.makeSquentialEdits):
            ...
            if self.curr_edit_idx % 20 == 0:
                # Create directories if they don't exist
                import os
                save_dir = os.path.join('image_logs', f'step_{step}')
                os.makedirs(save_dir, exist_ok=True)
                # Save original and rendered images before editing
                self._save_image(original_image[0], os.path.join(save_dir, f'original_{idx}.png'))
                self._save_image(rendered_image[0], os.path.join(save_dir, f'rendered_{idx}.png'))
                # Save edited image
                self._save_image(edited_image[0], os.path.join(save_dir, f'edited_{idx}.png'))
    ...
    def _save_image(self, tensor, filepath):
        """Helper function to save a tensor as an image.
    
        Args:
        tensor (torch.Tensor): Image tensor in CxHxW format
        filepath (str): Path where to save the image
        """
        import torchvision.transforms.functional as TF
        import torch
    
        # Ensure the tensor is on CPU and detached from graph
