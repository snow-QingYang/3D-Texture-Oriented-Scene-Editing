        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                for rendered_output_name in rendered_output_names:
                    ...
                    if is_depth:
                        depth_dir = os.path.join(output_image_dir, 'depth_npy')
                ...
                os.makedirs(os.path.join(output_image_dir, 'render'), exist_ok=True)
                if output_format == "images":
                    if image_format == "png":
                        media.write_image(output_image_dir / 'render' / f"{camera_idx:05d}.png", render_image, fmt="png")
                        for key in render_image_dict.keys():
                            os.makedirs(os.path.join(output_image_dir, key), exist_ok=True)
                            media.write_image(output_image_dir / key / f"frame_{camera_idx+1:05d}.png", render_image_dict[key], fmt="png")
                    if image_format == "jpeg":
                        media.write_image(
                            output_image_dir / 'render' / f"{camera_idx:05d}.jpg", render_image, fmt="jpeg", quality=jpeg_quality
                        )
                        for key in render_image_dict.keys():
                            os.makedirs(os.path.join(output_image_dir, key), exist_ok=True)
                            media.write_image(
                                output_image_dir / key / f"frame_{camera_idx+1:05d}.jpg", render_image_dict[key], fmt="jpeg", quality=jpeg_quality
                            )
                if output_format == "video":
                    if writer is None:
...
class DatasetRender(BaseRender):
    ...
    def main(self):
        ...
        def update_config(config: TrainerConfig) -> TrainerConfig:
            ...
            assert isinstance(data_manager_config, FullImageDatamanagerConfig)
